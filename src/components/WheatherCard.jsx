...
const WheatherCard = ({ weather, temp }) => {
  const [isCelsiu, setIsCelsiu] = useState(true);

  const handleChanteTemp = () => {
    setIsCelsiu(!isCelsiu);
  };
  return (
    <article className="weather">
      ...
      <footer className="weather__footer">
        <h2 className="weather__temp">
          {isCelsiu ? `${temp?.celsius} °C` : `${temp?.farenheit} °F`}
        </h2>
        <button className="weather__btn" onClick={handleChanteTemp}>
          Change to {isCelsiu ? "F°" : "C°"}
        </button>
      </footer>
    </article>
  );
